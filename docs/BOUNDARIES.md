...
- Auth boundary: `POST /api/founderos/capabilities/check`, `POST /api/founderos/precommit/plan`, `POST /api/founderos/commit/execute`, `POST /api/founderos/commit/merge-pr`, `POST /api/founderos/orchestrate/submit`, `GET /api/founderos/orchestrate/jobs/{job_id}`, `GET /api/founderos/trading/candidates`, `POST /api/founderos/trading/candidates/shadow-scan`, `GET /api/founderos/trading/candidates/{candidate_id}`, `POST /api/founderos/trading/candidates/{candidate_id}/decision`, `GET /api/founderos/trading/journal`, `GET /api/founderos/trading/backtests/{run_id}`, and `GET /api/founderos/trading/connectors/health` require the public APS key with server-side secret validation. `FOUNDEROS_PUBLIC_WRITE_KEY` is preferred; `FOUNDEROS_WRITE_KEY` remains a compatibility fallback for the same public lane. The canonical header is `x-founderos-key`; `Authorization: Bearer <key>` is tolerated only as a compatibility fallback for the same secret. `GET /api/founderos/capabilities` is intentionally public and read-only.
